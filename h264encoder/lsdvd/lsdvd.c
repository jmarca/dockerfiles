 ...
 */
static void converttime(playback_time_t *pt, dvd_time_t *dt)
{
	double fps = frames_per_s[(dt->frame_u & 0xc0) >> 6];
...
static char* lang_name(char* code)
{
	int k=0;
...
{
	fprintf(stderr, "lsdvd "VERSION" - GPL Copyright (c) 2002-2005, 2014 \"Written\" by Chris Phillips <acid_kewpie@users.sf.net>\n");
}
		...
		if ( opt_t == j+1 || opt_t == 0 ) {
			...
			if (ifo[ifo_zero->tt_srpt->title[j].title_set_nr]->vtsi_mat) {
				...
				if(pgc->cell_playback == NULL || pgc->program_map == NULL) {
					dvd_info.titles[j].general.length = 0;
					dvd_info.titles[j].general.playback_time.hour = 0;
					dvd_info.titles[j].general.playback_time.minute = 0;
					dvd_info.titles[j].general.playback_time.second = 0;
					dvd_info.titles[j].general.playback_time.usec = 0;
					dvd_info.titles[j].chapter_count = 0;
					dvd_info.titles[j].cell_count = 0;
					dvd_info.titles[j].audiostream_count_reported = 0;
					dvd_info.titles[j].audiostream_count = 0;
					dvd_info.titles[j].subtitle_count_reported = 0;
					...
					dvd_info.titles[j].audiostream_count_reported = vtsi_mat->nr_of_vts_audio_streams;
					dvd_info.titles[j].audiostream_count = 0;
					for(k = 0; k < dvd_info.titles[j].audiostream_count_reported; k++) {
						if((pgc->audio_control[k] & 0x8000) != 0) {
							dvd_info.titles[j].audiostream_count++;
						}
					}
					dvd_info.titles[j].subtitle_count_reported = vtsi_mat->nr_of_vts_subp_streams;
					dvd_info.titles[j].subtitle_count = 0;
					for(k = 0; k < dvd_info.titles[j].subtitle_count_reported; k++) {
				...
				if (opt_P) {
					...
					for (i=0; i < palsize; i++) { dvd_info.titles[j].palette[i] = pgc->palette[i]; }
				} else {
				...
				if (opt_a) {
					...
					for (i=0, k=0; i<dvd_info.titles[j].audiostream_count_reported; i++)
					{
						...
						dvd_info.titles[j].audiostreams[k].streamid = audio_id[audio_attr->audio_format] + i;
						k++;
					}
				} else {
				...
				if (opt_s) {
					for (i=0, k=0; i<dvd_info.titles[j].subtitle_count; i++)
					{
						if ((pgc->subp_control[k] & 0x80000000) == 0)
							continue;
						...
						dvd_info.titles[j].subtitles[k].streamid = 0x20 + i;
						k++;
					}
				} else {
